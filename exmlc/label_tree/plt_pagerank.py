...
class PLTClassifierPageranked(BaseEstimator):
    """
    TODO docs
    """
    ...
    def _compute_label_probabilities(self, y: Union[np.ndarray, csr_matrix]) -> Dict[int, float]:
        ...
        """
        edge_map = LabelCooccurrenceGraphBuilder(weighted=True, include_self_edges=False).transform(y)
        graph = nx.Graph()
        nodes = set()
        for entry in edge_map.items():
            node_pair = entry[0]
            if isinstance(node_pair, tuple):
                graph.add_nodes_from(node_pair)
                nodes.update(node_pair)
                graph.add_edge(*node_pair, weight=edge_map[node_pair])
            elif isinstance(node_pair, int):
                graph.add_node(node_pair)
                nodes.add(node_pair)
